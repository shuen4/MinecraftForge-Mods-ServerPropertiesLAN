...
public class splan {
    ...
    public boolean stop_LAN() {
        try {
            ...
            ThreadLanServerPing lsp=(ThreadLanServerPing) f.get(server);
            lsp.interrupt();
            f.set(server,null);
    ...
    public void SendMessageToPlayer(EntityJoinWorldEvent event) {
        ...
        if (event.getEntity() instanceof net.minecraft.entity.player.EntityPlayer && !sent) {
            ...
            event.getEntity().sendMessage((ITextComponent)new TextComponentString("allow-nether = " + bisNetherEnabled));
            event.getEntity().sendMessage((ITextComponent)new TextComponentString("enable-command-block = " + bisCommandBlockEnabled));
